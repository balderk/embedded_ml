...
if __name__ == '__main__':
    ...
    for model_name, in df[name_col].values:
        ...
        rae = relative_absolute_error(test_t, pred)
        rae_dict[model_name] = rae
        loc = np.where(df[name_col] == model_name)[0]

        for col_name, col in zip(target_keys, rae.T):
            df.loc[loc, f'test {col_name} rae 90'] = np.percentile(col, 90)
            df.loc[loc, f'test {col_name} rae 99'] = np.percentile(col, 99)
            df.loc[loc, f'test {col_name} rae 99.9'] = np.percentile(col, 99.9)
            print(f'{col_name} {np.percentile(col, 90)}, {np.percentile(col, 99)}, {np.percentile(col, 99.9)}')

        df.loc[loc, 'test rae 90 sum'] = np.sum(np.percentile(rae, 90, axis=0))
        df.loc[loc, 'test rae 99 sum'] = np.sum(np.percentile(rae, 99, axis=0))
        df.loc[loc, 'test rae 99.9 sum'] = np.sum(np.percentile(rae, 99.9, axis=0))
    df.to_csv('altered_results.csv', index=False)

    print(df.sort_values('test rae 99.9 sum', ascending=True).head(10))

    plt.boxplot(rae_dict[df.sort_values('test rae 99.9 sum', ascending=True).head(1)['name'].values[0]])
    plt.figure()
    sc = StandardScaler()
    vec = rae_dict[df.sort_values('test rae 99.9 sum', ascending=True).head(1)['name'].values[0]].T
    vec = sc.fit_transform(vec)
    for val in vec:
        print(val)
        sns.distplot(val)

    for k, v in rae_dict.items():
