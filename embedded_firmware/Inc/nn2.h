...
#define AI_NN2_MODEL_NAME          "nn2"

#define AI_NN2_IN_NUM       (1)
#define AI_NN2_IN { \
  AI_BUFFER_OBJ_INIT(AI_BUFFER_FORMAT_FLOAT, 1, 1, 8, 1, NULL), \
}
#define AI_NN2_IN_SIZE { \
  (1 * 1 * 8), \
}
...
#define AI_NN2_OUT_SIZE { \
  (1 * 1 * 4), \
}
#define AI_NN2_OUT_1_SIZE  (1 * 1 * 4)
#define AI_NN2_OUT_1_SIZE_BYTES  ((1 * 1 * 4) * 4)
 ...
 * @ref ai_nn2_create. Both the activations memory buffer 
 ...
 * header generated by the code generation tool. See AI_NN2_*
 * defines into file @ref nn2.h for all network sizes defines
 *
 * @param network an opaque handle to the network context
