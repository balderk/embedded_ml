...
#define AI_NET_OBJ_INSTANCE g_tanh_64

#undef AI_TANH_64_MODEL_SIGNATURE
#define AI_TANH_64_MODEL_SIGNATURE     "84e3a141a6b756fbcfcb9df4b7da8eb2"
...
#define AI_TOOLS_DATE_TIME   "Sun Feb 23 00:46:09 2020"

#undef AI_TOOLS_COMPILE_TIME
#define AI_TOOLS_COMPILE_TIME    __DATE__ " " __TIME__

#undef AI_TANH_64_N_BATCHES
#define AI_TANH_64_N_BATCHES         (1)
...
AI_NETWORK_OBJ_DECLARE(
        ...
        AI_BUFFER_OBJ_INIT(AI_BUFFER_FORMAT_U8,
                           1, 1, 512, 1,
                           NULL),
        AI_TENSOR_LIST_IO_ENTRY(AI_FLAG_NONE, AI_TANH_64_IN_NUM, &input_0_output),
        AI_TENSOR_LIST_IO_ENTRY(AI_FLAG_NONE, AI_TANH_64_OUT_NUM, &dense_3_output),
        &batch_normalization_layer, 0, NULL)


AI_DECLARE_STATIC
ai_bool tanh_64_configure_activations(
...
AI_DECLARE_STATIC
ai_bool tanh_64_configure_weights(
...
AI_API_ENTRY
ai_bool ai_tanh_64_get_info(
        ai_handle network, ai_network_report *report) {
    ai_network *net_ctx = AI_NETWORK_ACQUIRE_CTX(network);

    if (report && net_ctx) {
        ai_network_report r = {
                .model_name        = AI_TANH_64_MODEL_NAME,
                .model_signature   = AI_TANH_64_MODEL_SIGNATURE,
...
AI_API_ENTRY
ai_error ai_tanh_64_get_error(ai_handle network) {
    return ai_platform_network_get_error(network);
}

AI_API_ENTRY
ai_error ai_tanh_64_create(
...
AI_API_ENTRY
ai_handle ai_tanh_64_destroy(ai_handle network) {
    return ai_platform_network_destroy(network);
}

AI_API_ENTRY
ai_bool ai_tanh_64_init(
        ai_handle network, const ai_network_params *params) {
    ai_network *net_ctx = ai_platform_network_init(network, params);
    if (!net_ctx) return false;

    ai_bool ok = true;
    ok &= tanh_64_configure_weights(net_ctx, &params->params);
    ok &= tanh_64_configure_activations(net_ctx, &params->activations);
...
AI_API_ENTRY
ai_i32 ai_tanh_64_run(
        ai_handle network, const ai_buffer *input, ai_buffer *output) {
    return ai_platform_network_process(network, input, output);
}

AI_API_ENTRY
ai_i32 ai_tanh_64_forward(ai_handle network, const ai_buffer *input) {
