...
#define AI_NET_OBJ_INSTANCE g_relu_2_8

#undef AI_RELU_2_8_MODEL_SIGNATURE
#define AI_RELU_2_8_MODEL_SIGNATURE     "190d716395660d0d798552bdb207d88d"
...
#define AI_TOOLS_DATE_TIME   "Thu Feb 20 14:49:45 2020"

#undef AI_TOOLS_COMPILE_TIME
#define AI_TOOLS_COMPILE_TIME    __DATE__ " " __TIME__

#undef AI_RELU_2_8_N_BATCHES
#define AI_RELU_2_8_N_BATCHES         (1)
...
AI_NETWORK_OBJ_DECLARE(
        ...
        AI_BUFFER_OBJ_INIT(AI_BUFFER_FORMAT_U8,
                           1, 1, 512, 1,
                           NULL),
        AI_TENSOR_LIST_IO_ENTRY(AI_FLAG_NONE, AI_RELU_2_8_IN_NUM, &input_0_output),
        AI_TENSOR_LIST_IO_ENTRY(AI_FLAG_NONE, AI_RELU_2_8_OUT_NUM, &dense_3_output),
        &batch_normalization_layer, 0, NULL)


AI_DECLARE_STATIC
ai_bool relu_2_8_configure_activations(
...
AI_DECLARE_STATIC
ai_bool relu_2_8_configure_weights(
...
AI_API_ENTRY
ai_bool ai_relu_2_8_get_info(
        ai_handle network, ai_network_report *report) {
    ai_network *net_ctx = AI_NETWORK_ACQUIRE_CTX(network);

    if (report && net_ctx) {
        ai_network_report r = {
                .model_name        = AI_RELU_2_8_MODEL_NAME,
                .model_signature   = AI_RELU_2_8_MODEL_SIGNATURE,
...
AI_API_ENTRY
ai_error ai_relu_2_8_get_error(ai_handle network) {
    return ai_platform_network_get_error(network);
}

AI_API_ENTRY
ai_error ai_relu_2_8_create(
...
AI_API_ENTRY
ai_handle ai_relu_2_8_destroy(ai_handle network) {
    return ai_platform_network_destroy(network);
}

AI_API_ENTRY
ai_bool ai_relu_2_8_init(
        ai_handle network, const ai_network_params *params) {
    ai_network *net_ctx = ai_platform_network_init(network, params);
    if (!net_ctx) return false;

    ai_bool ok = true;
    ok &= relu_2_8_configure_weights(net_ctx, &params->params);
    ok &= relu_2_8_configure_activations(net_ctx, &params->activations);
...
AI_API_ENTRY
ai_i32 ai_relu_2_8_run(
        ai_handle network, const ai_buffer *input, ai_buffer *output) {
    return ai_platform_network_process(network, input, output);
}

AI_API_ENTRY
ai_i32 ai_relu_2_8_forward(ai_handle network, const ai_buffer *input) {
